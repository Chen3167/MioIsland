...
`设置 → 通用 → Anthropic API Proxy` 可以让 Code Island 查询 Anthropic 用量额度时走你本地的 HTTP 代理(比如 `http://127.0.0.1:7890`)。在本地跑 Clash / V2Ray 的开发者,直连 `api.anthropic.com` 不稳时用得上。

**作用范围注意:**
- ✅ 作用于刘海里的额度条(`RateLimitMonitor` → `api.anthropic.com/api/oauth/usage`)
- ❌ **不**作用于 Stats 插件的 "Editor's Note" AI 日报。那个功能是通过 fork 本地 `claude` CLI 子进程实现的,它的网络栈独立 —— 走你 shell 的 `HTTPS_PROXY` 环境变量 / `launchctl setenv HTTPS_PROXY` / 系统级代理(比如 Clash TUN 模式)
- ❌ 不作用于 CodeLight iPhone 同步(我们自己的服务器,直连即可)

如果想让 Stats 的 AI 日报也走同一个代理,最简单的做法是在登录时跑一次:
```bash
launchctl setenv HTTPS_PROXY http://127.0.0.1:7890
launchctl setenv HTTP_PROXY  http://127.0.0.1:7890
```
这会给所有从 Dock / Finder 启动的 GUI app 设好环境变量,`claude` CLI 子进程会自动继承。
